...
from differentiable_robot_model.data_generation_utils import generate_sine_motion_forward_dynamics_data
...
q = torch.Tensor([[-0.492, -0.828, -1.862,  0.163, -1.754,  0.714, -2.197]])#.repeat((10, 1))
qd = torch.Tensor([[ 4.406, -7.065, -3.089, -1.616,  7.562, -1.654, -6.038]])#.repeat((10, 1))
...
train_data = generate_sine_motion_forward_dynamics_data(gt_robot_model, n_data=10000, dt=1.0/250.0, freq=0.1)
...
for i in range(100):
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        qdd_pred = learnable_robot_model.compute_forward_dynamics(q=q, qd=qd, f=tau, include_gravity=True, use_damping=True)
